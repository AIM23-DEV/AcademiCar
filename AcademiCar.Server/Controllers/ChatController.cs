...
public class ChatController : ControllerBase
{
    ...
    public async Task<IActionResult> GetGroupChats(string id)
    {
        ...
        List<TripPassenger> passengerTripList =
            await _globalService.TripPassengerService.GetConnectionByPassengerID(id);
        foreach (TripPassenger tripPassenger in passengerTripList)
        {
            List<GroupChat> groupChatsForTrip =
                await _globalService.GroupChatService.GetGroupChatsByTripId(tripPassenger.FK_Trip);
            groupChatList.AddRange(groupChatsForTrip);
        }
    ...
    public async Task<IActionResult> GetPersonalChats(string id)
    {
        ...
        List<PersonalChat> driverPersonalChatList =
            await _globalService.PersonalChatService.GetPersonalChatsByDriverId(id);
        List<PersonalChat> passengerPersonalChatList =
            await _globalService.PersonalChatService.GetPersonalChatsByPassengerId(id);
        ...
        foreach (var personalChat in personalChatList)
        {
            ...
            List<PersonalMessage> personalMessageList = _globalService.UnitOfWork.PersonalMessages
                .FilterBy(message => message.FK_PersonalChat == personalChat.ID).ToList();
            if (personalMessageList.Any())
                personalChat.LastMessageContent =
                    personalMessageList.OrderBy(message => message.SentAt).Last()?.Content;
            else
                personalChat.LastMessageContent = "";
        }
